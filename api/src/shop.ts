...
router.post("/orders", async (req: Request, res: Response) => {
   try {
      ...
      const order = await db.transaction(async (trx) => {
         ...
         // inserting email into orders table to create new order and get order id.
         const [newOrder] = await trx
            .insert(orders)
            .values({ customer_email: email })
            .returning();

         // getting product prices for each product in order
         ...
         // insert each order item into order items table
         const orderProducts = await Promise.all(
            orderBody.map(async (orderItem: any, index: number) => {
               const total = (
                  +productPrices[index] * +orderItem.quantity
               ).toFixed(2);
